...
class Model(nn.Module):
    def __init__(self, in_dim=4, n_channels=32, output_dim=3, skip=False):
        ...
        # Image embedding for unconditional input
        self.uncond_embedding = nn.Parameter(torch.zeros(3, 256, 256))
    ...
    def forward(self, t, x_t, y=None, cond_mask=None):
        ...
        if cond_mask is not None:
            # cond mask is a boolean tensor of shape (batch_size,) indicating for each sample whether to mask or not
            assert cond_mask.shape[0] == x_t.shape[0], "cond_mask must have the same batch size as x_t"
            for i in range(x_t.shape[0]):
                if not cond_mask[i]:
                    x_t[i] = x_t[i] * y[i] + (1 - y[i]) * self.uncond_embedding
